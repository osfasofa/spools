...
**Status: pending sign-off.** This is a planning brief, not a decision. Nothing below lands in SPEC.md, DESIGN_DOC §5, or code until the owner answers §6's questions. It exists because relay persistence is the parked v2 fork (DESIGN_DOC §6), CLAUDE.md requires trade-off depth before protocol-shaping decisions, and this one shapes the protocol more than anything since encryption.
...
- **Hold:** the relay keeps the **last K deposits per room** (K≈4), each with a timestamp, TTL'd (~60 days), byte-capped. It's a pocket, not an archive: devices remain the spool's home.
...
- **Poisoning is bounded by physics already on the books.** The room code is a rendezvous name, not a secret (SPEC §1), so anyone holding the code (not the key) can PUT garbage. They can already spam the live room; the pocket makes spam durable, so: size caps, per-room rate limits, and — since the relay *cannot* verify ciphertext, by design — verification happens at the client, which drops-and-counts undecryptable deposits through the same surface as `on('undecryptable')`. A garbage deposit costs one wasted fetch, never a corrupt doc (nothing unauthenticated is ever handed to Yjs — SPEC §4's rule, reapplied).
...
- **TTL needs a repopulation rule.** Deposits expire; live spools must self-refresh. On open, after merging pocket and local state, a client deposits if it holds anything the pocket lacks ("deposit-if-ahead") — so a spool that keeps being *opened* keeps being covered, and a spool nobody opens for 60 days ages out of the pocket while remaining forever on its devices.
- **K=1 is not enough.** Each deposit is one writer's worldview. Two writers diverging offline → their latest deposits each lack the other's entries; keep only the newest and a fresh joiner merges one side until a live reunion. A small ring (K≈4) lets the joiner union all recent worldviews — and it self-collapses: the first client to merge them deposits the union, which supersedes the ring. T-100 must demonstrate the K=1 failure and the K=4 recovery.
...
- **Deposit blob:** `0xE2 0xE3 ‖ version(1 byte, =1) ‖ nonce(24) ‖ ciphertext` — secretbox over `Y.encodeStateAsUpdate(doc)`, same key-as-is, no KDF, continuing the magic family (`0xE2E1` wire, `0xE2E2` at rest, `0xE2E3` deposited). Version byte per the export-file lesson: readers MUST reject a version they don't understand.
- **Endpoints** (same origin as `/yjs`, the one-URL convention extended):
  - `PUT /pocket/<room>` — body: one deposit blob, `application/octet-stream`. Responses: `200 {"format":"spool-pocket","version":1,"stored":true}`, `413` too big, `429` rate-limited.
  - `GET /pocket/<room>` — `200 {"format":"spool-pocket","version":1,"deposits":[{"at":<ms>,"blob":"<base64>"}, …]}`, newest first, at most K. Base64 inflation accepted at intimate scale; JSON keeps it curl-able and makes the old-relay trap detectable.
  - Health JSON (already served on every path) gains `"pocket":{"rooms":N,"deposits":M,"ttlDays":60,"maxBytes":…}` — its **absence is the capability probe**.
  - Anything at these paths returning 200 *without* `"format":"spool-pocket"` = old relay = no capability. Never "empty pocket".
- **Relay storage:** per room, a ring of ≤K deposits (evict oldest on the K+1th PUT), swept by TTL. Default in-memory (npx-and-done stays npx-and-done, degrading to v1 semantics on restart); `POCKET_DIR` makes it a directory of blobs — no database, files named `<room>/<timestamp>`. Knobs: `POCKET_DIR`, `POCKET_TTL_DAYS` (60), `POCKET_MAX_BYTES` (8 MB, matching `MAX_FRAME_BYTES`), `POCKET_K` (4). The relay still imports neither yjs nor y-websocket — deposits are opaque files; the existing grep-proof keeps passing.
- **SDK, fetch side (T-102):** on open — probe capability (cached per relay origin) → after `whenReady` (which stays purely local, unchanged) → GET, verify envelope, decrypt each deposit, drop-and-count failures, `Y.applyUpdate` each survivor with a distinct origin tag. Emits an additive `pocket` event (`checking` / `applied` / `empty` / `unavailable`) — the `SpoolStatus` union stays closed.
- **SDK, deposit side (T-103):** a scheduler mirroring `HistoryLog` (`history.ts:96-105,174-193`): armed only after `whenReady`, listens to `afterTransaction`, **gated on `tr.local`** — pocket-applied updates arrive as remote-origin transactions, so applied state can never schedule a re-deposit; no self-feeding loop by construction. Debounce ~10 s, min gap ~60 s (deposits are heavier than moments). `leave()` awaits a final deposit before engine teardown, beside `history.flush()`; `visibilitychange → hidden` triggers a best-effort early flush (a real PUT, not a beacon — see the 64 KiB note). Deposit-if-ahead on open implements TTL repopulation.
...
- No identity, no accounts, no auth, no addressing. A deposit has no author — it's a worldview, not a message.
...
|---|---|---|---|
| T-100 | **Spike:** pocket feasibility — midnight loop headless in vitest (incl. cold open via fake-indexeddb), K=1 divergence failure + K=4 recovery, old-relay 200-trap caught by envelope, **frame-log noise quantified** (retire A on numbers, T-003-style results table) | — | everything below |
| T-101 | Relay: pocket capability — endpoints, K-ring, TTL sweep, caps, env knobs, memory default + `POCKET_DIR`, health block, no-yjs grep-proof kept | T-100 | |
| T-102 | SDK: fetch-on-open — capability probe, envelope verify, drop-and-count, merge after `whenReady`, additive `pocket` event | T-100 | |
| T-103 | SDK: deposit scheduler — `tr.local` gate, debounce/min-gap, `leave()` final flush, `visibilitychange`, deposit-if-ahead; midnight integration test | T-102 | |
...
| T-106 | SPEC v1.1 amendment (optional pocket capability + envelope), honesty-clause extension (the relay now *holds* ciphertext and observes deposit sizes/times), SDK-API.md, DESIGN_DOC §6 resolution + §5 rows | T-103 | spec written last, from working code |
...
4. Two writers diverge offline; a third device cold-opens and holds the union (the K-ring test).
