...
**First fork: agent workspaces** (owner-directed, Aug 2026). A shared spool between a person and a long-running software agent: the agent winds `finding` / `question` / `decision` entries as it works; the human steers by threading replies from a phone; `rewind` is the audit trail; the link is a per-task capability with zero account setup; export is the mission record. The quietly remarkable part: the room's conventions already cover it — *a seat is a device, not a person* was written without agents in mind and accommodates them without a word changing, and the §6 seat ladder (seat id → signing key, additive `sig`) would give agent attribution the same way it would give anyone attribution. Expected protocol pressure: **zero** — and if any appears, it queues at the gate like everything else. **Named by the owner (Aug 2026): `pfam`.** Repo `osfasofa/pfam`, born after T-130 per the gate; the charter beyond this paragraph lives there. Candidates passed over, for the record: `familiar`, `scribe`, `logbook`. Name diligence, T-002 style (knowing, not buying): npm `pfam` free as of Aug 2026 (registry 404 — likely never needed; the fork is an app, not a package). Faint collision: Pfam, the protein-families database — a different world entirely, noted and dismissed, same as the pocket's.
