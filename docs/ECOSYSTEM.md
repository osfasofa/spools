...
**First fork: agent workspaces** (owner-directed, Aug 2026). A shared spool between a person and a long-running software agent: the agent winds `finding` / `question` / `decision` entries as it works; the human steers by threading replies from a phone; `rewind` is the audit trail; the link is a per-task capability with zero account setup; export is the mission record. The quietly remarkable part: the room's conventions already cover it — *a seat is a device, not a person* was written without agents in mind and accommodates them without a word changing, and the §6 seat ladder (seat id → signing key, additive `sig`) would give agent attribution the same way it would give anyone attribution. Expected protocol pressure: **zero** — and if any appears, it queues at the gate like everything else. **Named by the owner (Aug 2026): `familiar`** — an agent bound to a person, keeping their spool; it also sits comfortably beside the lore vessel's world. Repo `osfasofa/familiar`, born after T-130 per the gate; the charter is drafted at [`docs/forks/familiar.md`](forks/familiar.md) (M13, third session) and migrates with the repo at birth. Naming log, per tradition: `pfam` was chosen first and reversed the same day ("a little weird"); `scribe` and `logbook` passed over. Diligence, T-002 style (knowing, not buying): npm `familiar` is taken by a dormant git-helper CLI — noted and dismissed, since the fork is an app, not a package; if it ever publishes one, it scopes or renames then.
