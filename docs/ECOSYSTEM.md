...
| **Off-grid kit** — [the brief](vessels/off-grid.md) | One laptop runs the relay; everyone else joins over LAN from a USB-stick client. Classrooms, field research, cabins, disaster response. | The most differentiated capability in the repo — collaboration with **no internet at all** — an accident of discipline promoted to a promise. Sneakernet (export → walk → import) proven as the third transport. Mostly packaging + a README that treats no-internet as the first-class scenario. | weekend |
| **Correspondence game (chess first)** — [the brief](vessels/chess.md) | Two people, one game, moves over days — the postcard revived. `kind: 'move'`, SAN + FEN in `data`; replay is the entry list itself. | Views-are-skins proof #3; the all-`data` vessel (no bodies at all, T-030 exercised downstream); turn-taking as *convention without permission* — the client never offers an illegal move, renders a rogue one visibly, and is honest that nothing stops it being wound. Chess: five centuries of prior art for §1's "trust, not proof." | weekend |
| **Memorial spool** (*shelved riff, Aug 2026 — candidate name: **wake**, the vigil and what a vessel leaves behind it*) | A family winds memories and stories for someone who died; when it's done, the export is the keepsake. `rewind` here is watching the remembering happen — the wake as it unfolded. | Spools that **end** — the finish → export → keep ceremony as a designed arc, not an afterthought. The gravest test of tone a client will ever face; its honesty sentence is link hygiene, because grief is where "hand this carefully" matters most. | weekend-plus, mostly design |
| **Quiet pad** (*shelved riff, Aug 2026 — candidate name: **carbon**, as in carbon paper: the copy exists on both sides by the act of writing, which is exactly what a Yjs binding over `spool.doc` is*) | Two or three people write in one ProseMirror/CodeMirror surface; the infrastructure structurally cannot read it. | The `spool.doc` escape hatch and the "every Yjs binding already works" claim, in anger. **Deliberately surfaces the yjs peer-dependency question** (first vessel that imports `yjs` itself — see RELEASING.md). | weekend-plus |
| **Care circle** (*shelved riff, Aug 2026*) | Siblings coordinate a parent's meds, visits, observations. | List+log hybrid views; E2E as the *point* rather than a feature (health data no one wants on a SaaS). Needs its own honesty sentence: a spool has no alarms and no guarantees — it is a shared notebook, never a medical device. Also where the keeper matters most practically: the sibling who runs the always-on peer holds the family's continuity. | room-class (deserves real care) |
...
2. **Off-grid kit** ([the brief](vessels/off-grid.md)) — cheapest differentiation available; almost pure packaging; makes a second true sentence no competitor can say.
3. **Correspondence chess** ([the brief](vessels/chess.md)) — small, joyful, and the third skin proof; also the first all-`data` vessel, which exercises the `T-030` verdict downstream.
...
**The bench beyond slot 3 is shelved (Aug 2026).** Next riffs queued when the brainstorm resumes: **wake** (memorial), **carbon** (quiet pad), the **care circle** — sketches folded into their rows above. The pick-up reference lives in [tickets/INDEX.md](../tickets/INDEX.md) §M13, which every session reads first.
...
**First fork: agent workspaces** (owner-directed, Aug 2026). A shared spool between a person and a long-running software agent: the agent winds `finding` / `question` / `decision` entries as it works; the human steers by threading replies from a phone; `rewind` is the audit trail; the link is a per-task capability with zero account setup; export is the mission record. The quietly remarkable part: the room's conventions already cover it — *a seat is a device, not a person* was written without agents in mind and accommodates them without a word changing, and the §6 seat ladder (seat id → signing key, additive `sig`) would give agent attribution the same way it would give anyone attribution. Expected protocol pressure: **zero** — and if any appears, it queues at the gate like everything else. **Named by the owner (Aug 2026): `familiar`** — the companion bound to one person, which is the fork's whole thesis in a word. The record here briefly read `pfam`; the owner reversed that call in favor of `familiar` (the original first candidate), and the correction is recorded here and in T-145's notes as an owner-directed supersession, not a silent rewrite (the T-144 rule). Repo `osfasofa/familiar`, born after T-130 per the gate — **satisfied: T-130 shipped 16 Aug 2026, registry-verified** — and the charter beyond this paragraph lives there, warm-started by the agents riff, which migrated with the repo when it was born (T-150, 18 Aug 2026) — [forks/familiar-riff.md](forks/familiar-riff.md) is now the pointer, and the riff's §8 stayed behind as the note below. Candidates passed over, for the record: `pfam` (named, then unnamed — its faint Pfam protein-families collision was noted and dismissed both times), `scribe`, `logbook`. Name diligence, T-002 style (knowing, not buying): npm `familiar` is a squatted 2022 package — irrelevant by this section's own reasoning (the fork is an app, not a package); the name that matters is the repo's.

**Evidence expected from the fork (the riff's §8, kept here).** Agents need nothing from the protocol or the SDK today — both spikes ran on shipped surface, and the fork's first session confirmed it a third time (`spools@0.1.0` straight from the registry, desk and leaves and the whole mission record, zero SDK asks). But the fork is the most likely **first real claimant** of two ladders DESIGN_DOC §6 banked years early, and the gates should expect the knock to come from there rather than from a vessel:

- **The `sig` field (§6 seat ladder).** Crypto attribution was deferred because humans at intimate scale don't need proof. Agents are natural signers — they never forget a key, and "which process wound this?" is operational, not social. The seat-id-as-future-pubkey opacity rule was written for exactly this arrival; when familiar wants verifiable agent testimony it arrives additively, as banked.
- **Read-only keys (§6 permissions ladder, rung 3).** The summarizer that should read and never write; the auditor that must not touch what it audits. Humans never really demanded this; agents will, because *structurally cannot write* is worth more than *instructed not to*, for the same reason sealed references beat traversal policies. Rung 3 breaks "one link, one key" — a rung, not a patch, and nothing moves without sign-off.

**And what the loom refuses regardless**, on the record beside the flags: no agent-shaped SDK surface without second-client evidence (nothing so far — the shipped surface has now sufficed three times); no bot flags in the protocol (unenforceable, therefore dishonest); no agent directories, marketplaces, or platforms (a directory is a feed wearing a trenchcoat, whoever's in it); and the white paper's sentence untouched — the roadmap stays people, and the fork stays another name. Fork reports rank at stranger's level: they queue at the parked-with-evidence gate, they don't jump it.
