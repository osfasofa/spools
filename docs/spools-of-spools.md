...
*(Rx's flattening operators name this menu — auto-traversal is `mergeAll`, one-at-a-time is `switchAll` — and the [tape-deck riff](riffs/tape-deck.md) §2 tabulates it; `spools-keeper --links` is `mergeAll` on purpose, the one peer that can afford every socket.)*
