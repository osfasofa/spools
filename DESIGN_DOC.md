...
- **Multi-writer beyond two people** — Yjs says it just works; verify with a three-writer test (concurrent winds, concurrent body edits, offline/rejoin) in the SDK test ticket. The remaining question is social, not technical: the link grants full write access to everything — including other people's entries — and that's the honest v1 contract.
...
- **Structured `data` payloads for immutable kinds** — re-parked with a cheap exit: a plain-JSON `data` field is purely additive later (clients ignore unknown fields; plain map values are last-write-wins, fine only for truly immutable data). The mixtape renderer (M3) decides whether it earns its spec sentence.
