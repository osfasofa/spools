...
| **splice** | Write complete entry records — identity, time, author, parent, data, body — into a spool exactly as given, in one transaction. `spool.splice(records)`. Idempotent (an id already present is skipped), and it refuses a record whose parent is missing: a dangling parent in a fresh spool would render as "not synced yet", a lie. The one primitive under the cut, the fork, and the rejoin; those stay recipes. *(M16, T-186.)* |
| **reel** | A spool seen as a length of tape: it has a beginning, it grows, and it can be cut. The result of a cut is a new reel — a new spool with a new key and no past before the cut. The owner's word (`docs/riffs/the-reel.md`). *(M16.)* |
| **cut** | The gesture: point at an entry and start a new reel from there on, splicing what you keep onto it. Client language, never an SDK verb; the SDK offers no time-cut, only records. Replies whose parent didn't make the cut become plain entries, said on the button. *(M16, T-187.)* |

- Package name: **`spools`** (verified available on npm as of Aug 2026; bare `spool` is a dead squat). Claim it with a 0.0.1 stub early.
- Naming rejected along the way, for the record: *yarn* (npm collision), *reel* (Instagram collision), *fable* (model-name mixup, never actually chosen), *bundle/parcel* (bundler collisions), *tape/track/cut* (music-locked). *Reel* and *cut* came back in M16 (Sep 2026) as vocabulary for the result and the gesture of forgetting, not as names for the thing itself — the collision was with the package name, and that stays `spools`.
...
- [x] **9. `splice` (M16)** — one primitive under the cut, the fork, and the rejoin; the room's cut, tape counter, and reel length on top. *(done Sep 2026, T-180 brief at [docs/M16-splice-brief.md](docs/M16-splice-brief.md), T-186–T-188; SPEC gained one non-normative sentence)*
- [ ] Later: encryption hardening pass, more renderers (board, blog).
...
| The splice family (M16) | **One primitive, `spool.splice(records)`** — writes complete `EntrySnapshot`-shaped records (`id`, `author`, `kind`, `createdAt`, `parent?`, `data?`, `deletedAt?`, `body`) into this spool exactly as given, in one transaction; **idempotent** (an id already present is skipped — a re-run is a byte-level no-op); **refuses a dangling parent** before any write (`SpoolSpliceError` naming the record and the rule) — a parent must resolve in the batch or in the target, soft-deleted parents included; policy-free otherwise (what crosses, provenance, the new key are the caller's). `Entry.snapshot()` is the selection step. **Fork and rejoin stay recipes** in SDK-API (two lines and the honesty sentence each) until a vessel ships reunion. **Names:** `splice` takes the reserved verb; *reel* and *cut* enter §2; `retell`/`fork`/`rejoin` are unreserved recipe names. **The room's cut flattens** replies whose parent didn't make the cut, said on the button. **A reserved reel-length kind** (advisory, newest-wins) and **the tape counter** ("full is a cut, not a wall", reading `pocket.maxBytes` from the link's relay, never a constant) are one room ticket. Zero protocol change; `wind()`, the link, the relay, the pocket, `gc:false` untouched | Three clients reached under the SDK to copy entries into a new spool (lore's v1, syrup's `/branch`, the room's T-164) for one reason in `entry.ts`: `wind()` stamps `id` *and* `createdAt`, so a retelling built from it loses identity and **order** (a loop's entries share a millisecond and sort by random id). Two independent wanters for identity across a retelling (tape-deck's feedback guard, the reel's cut). The primitive is the smallest surface that removes that reach and the only option that can refuse the "not synced yet" lie at the seam; three verbs would promise fork/rejoin with no shipper (the gate forbids promising); widening `wind()` would put forged time and identity on the naive client's one verb. Bakes in: an entry id can be *the same entry* in two spools (the feature — idempotence, order, cheap provenance; a future cross-spool index must know), and `EntrySnapshot` as an input type. Measured (reel spike, 5 000 messages): 760 KiB with ids preserved vs 965 KiB with a `from` stamp vs 1 594 KiB for the old reel half-hidden — identity is the cheaper provenance; a cut by time pulled six extra entries from `createdAt` ties, so the seam is by entry. The new reel has no past: `rewind()` there starts at the cut, loudly *(5 Sep 2026, docs/M16-splice-brief.md, owner-approved as recommended)*. **Shipped the same night:** T-186 (SDK, 13 tests, the recipes in SDK-API) and T-187 (room — its own conventions, recorded in that ticket: the reel length is in messages; `room:home` carries the old code and relay but never the key; **no `next` on the old reel** — the cut writes nothing to it, and neither sealing is honest: under the new key it is unreadable to the old reel's people, under the old key it hands the new key to whoever the cut may be leaving out); T-188 checked these words against the code |
| The address bar (M15, T-165) | **Option C: the client drops `k=` from the address bar once the stash confirms it holds the full link, and a bare link reopens through the stash.** The bar keeps `#spool=<code>&relay=…`; `share()`, the Settings copy button, and the stash row keep the key. A device whose storage swallowed the registry write keeps the key in the bar — it is then the only place the key lives. A bare link for a room the device never held opens keyless, and the room says so ("this link carries no key, and this device never held this room") instead of the generic wrong-key line. Shipped in the room and the mixtape (copied prose, per the apps' rule); the link grammar and SPEC untouched | Browsers sync their address bars, open tabs, and bookmarks to their makers — "never sent to a server" was true of *this project's* servers only (review finding F6). Option A (keep it) leaves the key in every sync; B (strip always) loses a room on a device whose storage is blocked and in private windows; C is B's protection with the stash as the guard, and the stash registry already holds the full link for every persisted room, so the guard is a read. Costs: a bookmark reopens the room only while the stash holds it, and copying from the URL bar no longer hands the key — Settings is the copy path (the LAN row says so). The T-165 sentence still travels with every copied link *(6 Sep 2026, owner-approved)* |
| Pocket defaults against strangers (M15, T-168) | **Canonical relay: `POCKET_NEW_NAMESPACES_PER_HOUR=60`; `POCKET_FIRST_MAX_BYTES` stays equal to `POCKET_MAX_BYTES` (off).** Eviction order (never-read first, then stalest) shipped in T-168 without a default change and stands | The creation cap is invisible to a household and has a real address to count since `TRUST_PROXY` went on (T-161); it is what protects the *uncollected* real deposit from a flood, which eviction order cannot. The first-deposit cap was refused: a spool built alone for a week and then shared would 413 on every deposit until one read, the SDK latches that as too-big for the session, and the friend's first cold open would find an empty pocket — the midnight case the pocket exists for. README's honesty sentence stands: a determined stranger can still fill the pocket; devices remain the spool's home *(6 Sep 2026, owner-approved; set on Railway at the owner's keyboard the same night — verified live 7 Sep)* |
| Link shape (M15, T-177) | **The room hands out `#spool=<code>&k=<key>`: `relay=` is dropped when — and only when — it equals the SDK's `DEFAULT_RELAY`** (~130 → ~95 characters). An app-level translation, in `apps/room/src/link.ts`: `spool.share()`, the stash row, and SPEC's *links SHOULD carry `relay`* are untouched, so the SDK still builds fully pinned links and the stash still reopens by one. A room on any other relay keeps carrying it, in the copy button, in the minted new-room/new-reel link, and in the address bar. **Refused, so it isn't relitigated:** any server-side namer or short-link service — it would hold keys, or the mapping to them (a directory is a feed in a trenchcoat; the relay must never see a key, SPEC §4) — and a shorter key (§5's no-KDF decision). The path form `/<code>#k=…` (~90) stays available and unbuilt; it needs the host to serve `index.html` for any path, which T-167 decides | The link's second-longest part was a URL-encoded hostname every canonical client already defaults to. Option 1 (leave it) keeps ~25 characters of nothing; 3 pins a hosting behaviour before T-167 answers; 4 (pick your own words) is a chooser, not a shape. What it buys beyond length: a link handed out today pins **no** hostname, so moving the canonical relay becomes a client redeploy instead of a stranded link. What it costs, stated in SPEC §1 already: the opening client's default now decides the rendezvous, so a client defaulting elsewhere and a chat.spools.lol link would not meet — which is why the omission is conditional on the relay *being* the default, and why vessels on their own relays are untouched. Smoke 26 proves both halves and that a short link still opens `wss://relay.spools.lol/yjs/<code>` *(7 Sep 2026, owner-approved)* |
| `stash.remember` (M16, T-179) | **Option A: the registry stamp made public — `stash.remember(code, link)`**, the row an open would have written (link + `lastOpened`), opening and contacting nothing. Throws `SpoolLinkError` on a code that isn't one, a link that isn't one, or a link naming a different spool. Option B (a `remember` flag on `openSpool`/`newSpool`) refused: it couples "don't persist the doc" to "do record the link" | syrup was rewriting `spools:stash` by hand — same key, same row shape — because its satchel opens with `persist: false` and `touch()` was private (its own comment asked for this function). A vessel coupled to a **private storage format** is worse evidence than the second client the parked-with-evidence rule asks for: any change to the registry shape would have broken it silently. One additive function, a patch release, and the shape stops being a de facto contract held outside this repo *(7 Sep 2026, owner-approved)* |
---
