...
| Mutable shared state (M11) | **Entries, not new root types**: chat name and profiles are reserved-kind entries (`room:*`), filtered from the message view, resolved newest-wins at render. Read receipts persist as **one `room:read` entry per seat whose body is rewritten** (bodies are mutable; metadata is write-once) — never one entry per read event, never per message | Keeps mutation inside the sanctioned model: naive clients render reserved kinds via the unknown-kind fallback, `rewind()` covers them, no private structure invented. This is the mutation-as-entry experiment M11 exists to run. **Open, priced by T-110:** a body rewrite is a local content transaction, so each cursor advance past the min-gap also appends a ~0.5 KB permanent history moment (history.ts:174) and dirties the pocket — ~6× the naive estimate; a lurker writes like an active writer. T-110's numbers decide throttle-hard vs ephemeral-only before T-121 builds on it *(Aug 2026, M11 brief, user-approved; D4 pricing pending evidence)* |
...
- **Chat-scale growth** — opened by M11, eyes open: `gc:false` + in-doc history (~0.5 KB/moment, no pruning path) + whole-doc pocket deposits + the 8 MiB frame/deposit cap mean a long-lived group chat eventually outgrows the transport, and the 413 latch degrades it silently to live-only. T-110 measures the crossing point; if the numbers demand pruning/compaction/`splice`, that conversation is protocol-shaping and comes back here with evidence. Related knob: the pocket ring's K=4 was sized for two-person spools and may need re-sizing at group cardinality (T-124, sign-off required).
