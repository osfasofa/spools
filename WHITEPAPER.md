...
- **The link is total, irrevocable capability.** Anyone holding it reads everything and writes anything — including editing others' entries and the whole past. There is no partial history and no read-only. One paste into the wrong chat is one paste too many. The clients say this before first wind; it is the price of no accounts, and you should decide it's worth it *before* you start.
...
- **A full room looks like a bad connection.** The relay guards at 64 connections per room; today's SDK experiences the 65th as an endless reconnect cycle rather than a clear error. Documented, unfixed, honest.
